...
class ds(nn.Module):
    def __init__(self, inplanes, outplanes, strides=1, k_size=3, padding=0):
        nn.Conv2d(inplanes, outplanes, kernel_size=k_size, stride=strides, bias=False, padding=padding)
...
class HighResolutionNet(nn.Module):
    def __init__(self, base_channel: int = 32, class_num: int = 6):
        ...
        self.ds = nn.Sequential(
            nn.Conv2d(32, 64, kernel_size=1, stride=1, bias=False, padding=0),
        ...
        self.ds2 = nn.Sequential(
            nn.Conv2d(64, 128, kernel_size=1, stride=1, bias=False, padding=0),
            nn.BatchNorm2d(128, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True)
        )
        self.ds3 = nn.Sequential(
            nn.Conv2d(512, 512, kernel_size=3, stride=2, bias=False, padding=1),
            nn.BatchNorm2d(512, momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True)
        )
