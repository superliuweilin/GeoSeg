...
from attention.FRF import CA, SA
...
class StageModule(nn.Module):
    def __init__(self, input_branches, output_branches, c, stage=2, rep=1):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.ca2_1 = CA(64, 32)
        self.ca2_2 = CA(64 * stages * 2, 64 * stages)
        self.ca3_1 = CA(64, 32)
        self.ca3_2 = CA(64 * stages * 2, 64 * stages)
        self.ca4_1 = CA(64, 32)
        self.ca4_2 = CA(64 * stages * 2, 64 * stages)
        self.sa2_1 = SA(kernel_size=7)
        self.sa2_2 = SA(kernel_size=7)
        self.sa3_1 = SA(kernel_size=7)
        self.sa3_2 = SA(kernel_size=7)
        self.sa4_1 = SA(kernel_size=7)
        self.sa4_2 = SA(kernel_size=7)
        self.upca2_1_1 = nn.Conv2d(c, 64, stride=1, kernel_size=1)
        self.upca2_1_2 = nn.Conv2d(c, 64, stride=1, kernel_size=1)
        self.dwca2_1_1 = nn.Conv2d(64, c, stride=1, kernel_size=1)
        self.dwca2_1_2 = nn.Conv2d(64, c, stride=1, kernel_size=1)
        self.upca2_2_1 = nn.Conv2d(64 * stages, 64 * stages * 2, kernel_size=1, stride=1)
        self.upca2_2_2 = nn.Conv2d(64 * stages, 64 * stages * 2, kernel_size=1, stride=1)
        self.dwca2_2_1 = nn.Conv2d(64 * stages * 2, 64 * stages, kernel_size=1, stride=1)
        self.dwca2_2_2 = nn.Conv2d(64 * stages * 2, 64 * stages, kernel_size=1, stride=1)
        self.upca3_1_1 = nn.Conv2d(c, 64, stride=1, kernel_size=1)
        self.upca3_1_2 = nn.Conv2d(c, 64, stride=1, kernel_size=1)
        self.dwca3_1_1 = nn.Conv2d(64, c, stride=1, kernel_size=1)
        self.dwca3_1_2 = nn.Conv2d(64, c, stride=1, kernel_size=1)
        self.upca3_2_1 = nn.Conv2d(64 * stages, 64 * stages * 2, kernel_size=1, stride=1)
        self.upca3_2_2 = nn.Conv2d(64 * stages, 64 * stages * 2, kernel_size=1, stride=1)
        self.dwca3_2_1 = nn.Conv2d(64 * stages * 2, 64 * stages, kernel_size=1, stride=1)
        self.dwca3_2_2 = nn.Conv2d(64 * stages * 2, 64 * stages, kernel_size=1, stride=1)
        self.upca4_1_1 = nn.Conv2d(c, 64, stride=1, kernel_size=1)
        self.upca4_1_2 = nn.Conv2d(c, 64, stride=1, kernel_size=1)
        self.dwca4_1_1= nn.Conv2d(64, c, stride=1, kernel_size=1)
        self.dwca4_1_2 = nn.Conv2d(64, c, stride=1, kernel_size=1)
        self.upca4_2_1 = nn.Conv2d(64 * stages, 64 * stages * 2, kernel_size=1, stride=1)
        self.upca4_2_2 = nn.Conv2d(64 * stages, 64 * stages * 2, kernel_size=1, stride=1)
        self.dwca4_2_1 = nn.Conv2d(64 * stages * 2, 64 * stages, kernel_size=1, stride=1)
        self.dwca4_2_2 = nn.Conv2d(64 * stages * 2, 64 * stages, kernel_size=1, stride=1)
        if stage == 3:
            self.ca2_1 = self.ca3_1
            self.sa2_1 = self.sa3_1
            self.ca2_2 = self.ca3_2
            self.sa2_2 = self.sa3_2
            self.upca2_1_1 = self.upca3_1_1
            self.upca2_1_2 = self.upca3_1_2
            self.dwca2_1_1 = self.dwca3_1_1
            self.dwca2_1_2 = self.dwca3_1_2
            self.upca2_2_1 = self.upca3_2_1
            self.upca2_2_1 = self.upca3_2_1
            self.dwca2_2_1= self.dwca3_2_1
            self.dwca2_2_2 = self.dwca3_2_2

        elif stage == 4:
            self.ca2_1 = self.ca4_1
            self.sa2_1 = self.sa4_1
            self.ca2_2 = self.ca4_2
            self.sa2_2 = self.sa4_2
            self.upca2_1_1 = self.upca4_1_1
            self.upca2_1_2 = self.upca4_1_2
            self.dwca2_1_1 = self.dwca4_1_1
            self.dwca2_1_2 = self.dwca4_1_2
            self.upca2_2_1 = self.upca4_2_1
            self.upca2_2_1 = self.upca4_2_1
            self.dwca2_2_1 = self.dwca4_2_1
            self.dwca2_2_2 = self.dwca4_2_2
    ...
    def forward(self, x):
        ...
        for i in range(len(self.fuse_layers)):
            ...
            # )
            # x_fused.append()
            x_cat = torch.cat([self.fuse_layers[i][j](x[j]) for j in range(len(self.branches))], 1)
            # print(x_cat.shape)
            if i == 0:
                ca_w = self.ca2_1(x_cat)
                sa_w = self.sa2_1(x_cat)
                sum_add = 0
                for j in range(len(self.branches)):
                    if j == 0:
                        x_low = self.fuse_layers[i][j](x[j])
                        # print('x_low.shape = {}'.format(x_low.shape))
                        x_low = self.upca2_1_1(x_low)
                        # print('x_low.shape = {}'.format(x_low.shape))
                        sum_add += self.dwca2_1_1(x_low * ca_w)
                    elif j == 1:
                        x_hig = self.fuse_layers[i][j](x[j])
                        # print('x_hig.shape = {}'.format(x_hig.shape))
                        x_hig = self.upca2_1_2(x_hig)
                        # print('x_hig.shape = {}'.format(x_hig.shape))
                        sum_add += self.dwca2_1_2(x_hig * sa_w)
                    # print('sum_aad.shape = {}'.format(sum_add.shape))
            elif i == 1:
                ca_w = self.ca2_2(x_cat)
                sa_w = self.sa2_2(x_cat)
                sum_add = 0
                for j in range(len(self.branches)):
                    if j == 0:
                        x_low = self.fuse_layers[i][j](x[j])
                        # print('x_low.shape = {}'.format(x_low.shape))
                        x_low = self.upca2_2_1(x_low)
                        sum_add += self.dwca2_2_1(x_low * ca_w)
                    elif j == 1:
                        x_hig = self.fuse_layers[i][j](x[j])
                        # print('x_hig.shape = {}'.format(x_hig.shape))
                        x_hig = self.upca2_2_2(x_hig)
                        sum_add += self.dwca2_2_2(x_hig * sa_w)
                    # print(sum_add.shape)

            x_fused.append(self.relu(sum_add))
