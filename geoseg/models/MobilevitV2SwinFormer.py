...
class Block(nn.Module):
    def __init__(self, dim=256, num_heads=16, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.ReLU6, norm_layer=nn.BatchNorm2d, window_size=8, depth=2):
        super().__init__()
        self.norm1 = norm_layer(dim)
        # self.attn = GlobalLocalAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias, window_size=window_size)
        self.attn =MobileViTAttention(in_channel=dim,dim=dim,depth=depth,mlp_dim=int(4*dim))
...
class MobilevitV2SwinFormer(nn.Module):
    def __init__(self,
                 decode_channels=64,
                 dropout=0.1,
                 pretrained=True,
