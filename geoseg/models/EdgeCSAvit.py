...
from geoseg.models.EdgeViT import edgevit_xs
...
class Decoder(nn.Module):
    def __init__(self,
        ...
        self.p1 = nn.Sequential(Conv(48, decode_channels,kernel_size=1))
...
class EdgeCSAvit(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.backbone = edgevit_xs()

        # encoder_channels = (64, 96, 128, 640)

        #Edgevit
        encoder_channels = (48, 96, 240, 384)
    ...
    def forward(self, x):
        h, w = x.size()[-2:]
        _, res1, res2, res3, res4 = self.backbone(x)
        if self.training:
